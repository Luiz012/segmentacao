...
void draw() {
  ...
  for (int y = 0; y < img.height; y++) {
    for (int x = 0; x < img.width; x++) { 
      int pos = (y)*img.width + (x);
       if(green(imgPB.pixels[pos]) < 70) seg_PB.pixels[pos] = color(255);
  ...
  for (int y = 0; y < img.height; y++) {
    for (int x = 0; x < img.width; x++) { 
      int pos = (y)*img.width + (x); 
       if(green(imgPB.pixels[pos]) > 170) seg_PA.pixels[pos] = color(255);
